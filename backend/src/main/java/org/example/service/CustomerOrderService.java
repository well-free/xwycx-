...
import org.example.infrastructure.outbox.CustomerOrderOutboxService;
...
public class CustomerOrderService {
    ...
    private final CustomerOrderOutboxService outboxService;
    ...
    public CustomerOrderService(CustomerOrderMapper orderMapper,
                                ...
                                CustomerOrderOutboxService outboxService,
    ...
    private CustomerOrderResponse createInTransaction(AuthUserResponse user,
        ...
        for (CustomerOrderItemRequest itemRequest : request.items()) {
            ...
            logInventory(product.getId(), orderId, -itemRequest.quantity(), itemRequest.quantity(), 0L,
                    "ORDER_RESERVE");
        }
        order.setTotalAmount(total.setScale(2, RoundingMode.HALF_UP));
        orderMapper.insert(order);
        cartService.removePurchasedLocked(user.id(), productIds);
        outboxService.enqueueTimeout(orderId, Duration.ofSeconds(properties.getOrder().getTimeoutSeconds()));
    ...
    public CustomerOrderResponse ship(AuthUserResponse admin, long orderId, ShipmentRequest request) {
        ...
        return lifecycleService.withOrderLock(orderId, () -> orderTransactionTemplate.execute(status -> {
            ...
            Instant now = Instant.now();
            lifecycleService.commitShipmentInventoryLocked(orderId);
    ...
    private void logInventory(long productId, long orderId, long quantityChange,
                              long reservedChange, long soldChange, String reason) {
        InventoryLogEntity log = new InventoryLogEntity();
        log.setId(IdWorker.getId());
        log.setProductId(productId);
        log.setOrderId(orderId);
        log.setQuantityChange(quantityChange);
        log.setReservedChange(reservedChange);
        log.setSoldChange(soldChange);
        log.setBusinessKey(reason + ":" + orderId + ":" + productId);
